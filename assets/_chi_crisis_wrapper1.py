...
def build_loc():
    ...
    add(
        loc_mod_line(
            ...
            "Селектор чинит дамбы сразу во всём регионе.\n"
            "Пять регионов: Южный Чжили, Хэнань, Нанкин, Шаньдун, Циндао.\n"
            "Прогресс — карточка страны «Дамбы Хуанхэ».",
        )
    )
    ...
    add(
        loc_line(
            ...
            "Казённый амбар региона. Снимает ступень голода. Сам прирост населения не даёт. Сельское хозяйство региона +5%.",
        )
    )
    ...
    yellow_sites = "Южный Чжили, Хэнань, Нанкин, Шаньдун, Циндао"
    for n in range(5):
        add(loc_line(f"CHI_yellow_problem_{n}", f"Дамбы Хуанхэ {n}/5"))
        add(
            loc_mod_line(
                f"CHI_yellow_problem_{n}_desc",
                f"Регионы с дамбами: {yellow_sites}.\n"
                f"Отремонтировано: {n} из 5.\n"
                f"{recog_bar(20 * n / 5)}\n"
                "Один заказ селектора закрывает весь регион. Успех всех пяти до 1845: ферма и добыча +20%, фабрики +15%. "
                "Если не успеть - решение двора снимет штрафы без награды.",
            )
        )
    ...
    add(loc_line("CHI_yellow_old_course", "Дамбы Хуанхэ 5/5"))
    add(
        loc_mod_line(
            "CHI_yellow_old_course_desc",
            "Все пять регионов на Хуанхэ укреплены в срок.\n"
            f"{recog_bar(20)}\n"
            "Южный Чжили, Хэнань, Нанкин, Шаньдун, Циндао.\n"
